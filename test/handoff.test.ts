...
describe("canonical handoff", () => {
  test("keeps Evidence optional and non-blocking for NEXT", () => {
    expect(StructuredHandoffSchema.properties.evidence.description).toContain("Write 'none' if empty");
    expect(StructuredHandoffSchema.properties.evidence.description).toContain("File paths, commands, IDs");
  });
