...
describe("ACM guidance quality", () => {
  test("unifies judgment layers on the working-set invariant", () => {
    ...
    expect(ACM_CORE).toContain("**Failure shapes** name what went wrong and the corrective lean");
    ...
    expect(ACM_CORE).toContain("Boundary decides whether folding is valid");
    ...
    expect(ACM_CORE).toContain("recover the needed detail (or its pointer) before continuing");
    expect(ACM_CORE).toContain("run a rebase check or fold the named boundary once criteria pass");
    expect(ACM_CORE).toContain("choose an earlier pre-boundary anchor");
