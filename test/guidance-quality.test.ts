...
describe("ACM guidance quality", () => {
  test("anchors normal workflow on smells and gates instead of required transitions", () => {
    ...
    expect(ACM_CORE).toContain("What becomes reasonable");
  ...
  test("front-loads a checkable managed-chain preflight", () => {
    ...
    expect(ACM_CORE).toContain("opens a **managed chain**");
    expect(ACM_CORE).toContain("First complete any required finished-chain transition");
    expect(ACM_CORE).toContain("confirm `<chain>-start` recoverability with `acm_checkpoint` before managed work begins");
    ...
    expect(TOOL_DESCRIPTIONS.checkpoint.startsWith("Confirm recoverability before managed work")).toBe(true);
