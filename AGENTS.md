...
7. 只在 branch 明确成功时 schedule persistent context refresh 与 per-SessionManager live AgentSession sync ticket；matching `tool_execution_end` 只关联 receipt，不得替换 native messages。originating assistant run 与其 automatic retry/tool loop 始终保留当前 live messages，完整保留刚发生的 tool-call/result 连续性；仅 `agent_settled` 才把 native AgentSession replacement 应用到最新 verified active leaf。`agent_end`（尤其 provider error）不是 release/apply signal。matching `tool_result` 在没有 pending later message 且 run 未 abort 时另发送一条 hidden `steer`，只重申 handoff 的 `next` 并把 pre-travel request 标为历史；有 pending/abort 时跳过 transient steer，依赖原位 Context Packet，让后来用户消息保持最高时序；失败或 indeterminate result 不 schedule refresh/sync，也不发送 steer
...
## Settled post-travel context refresh

successful travel 后，`ContextRefreshRegistry` 按 SessionManager identity 记录 pending refresh，同时 live adapter 记录同一 manager 的 pending ticket。matching `tool_execution_end` 只能确认该 tool pair；originating assistant run 及其 automatic retry/tool loop 内的 `context` event 必须保留 host 当前 live messages，不能以 traveled branch 替换它。只有 `agent_settled` 表明该 run 的连续性不再需要时，adapter 才从最新 verified active leaf 应用 native AgentSession replacement；`agent_end`（特别是 error/aborted terminal）不解锁该路径。随后 persistent rebuild 按以下步骤验证或恢复 Context Packet：
