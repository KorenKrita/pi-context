...
2. 运行统一 tool-protocol analysis，并在原消息位置把 marker 与 persisted ACM travel provenance 同时匹配的 branch summary 投影为 authoritative hidden continuation；legacy/native/foreign summary 保持 archival。只有与该 trusted summary 的 entry/target/origin/toolCall provenance 精确匹配、且 finalized details 明确为 non-error `mutationStatus: applied` 的孤立 ACM receipt 才作为安全 normalization 移除；foreign/untrusted receipt 与任何普通 repair 仍保持 repaired/invalid evidence
3. 成功后标记 rebuilt
