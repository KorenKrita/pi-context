...
| Commit | 作用 |
|---|---|
| `cc14926e` | 提交 post-merge pivot / restraint 矩阵 |
| `9f0e5ce5` | 让 pivot Recover scorer 对齐可验证的 handoff recovery route |
| `1640cce8` | 澄清 rehydrate return target 与 origin backup alias 的机械语义 |
| `8759f124` | 接受语义等价的 explicit deferred-wait wording |
| `77eb9380` | 把 durable artifact 完整性证据固定在 travel 前的 write / shell 内容 |

Live 行为运行使用本机 Pi CLI **0.80.10**；deterministic host fixture 继续严格固定 **Pi 0.80.7**。二者分开记录。
...
Outgoing review 发现了一个时间归因缺口：旧实现只确认 travel 前出现过 write，然后用 scenario 结束后的最终文件证明“当时已经完整”。`77eb9380` 改为同时要求：

- travel 前成功 write 的 `content`，或成功 shell redirect/tee 的 `command`，已经携带三个完整 section；
...
- `30f754e3` 与生产提交 `1640cce8` 在 `TOOL-CONTRACTS.md`、`src/generated-guidance.ts`、`src/travel-tool.ts` 上逐字相同；差异只在 prototype 测试名，不进入 runtime prompt surface。
...
该 raw batch 记录的 `productCommit` 是 `497d9885`。它与最终 `8759f124` 的 `src/`、`skills/`、`package.json`、`package-lock.json` 完全相同；两者只差后续 deferred-wait evaluator wording，因此行为产品面一致。Committed matrix source 是 `eval/matrix.optimization-final.mjs`，随后由 `00b10e26` 固化。
...
Haiku 的一条 raw failure 使用了语义正确的 `Wait for explicit next user instruction`，当前 `8759f124` scorer 接受；另一条真实失败把 return alias 同时放进 `target` 和 `backupCurrentHeadAs`，随后无 backup retry 成功。
...
```text
bun run verify:acm
root: 279 pass / 0 fail
exact Pi 0.80.7 host fixture: 53 pass / 0 fail
typecheck: pass
...
独立 reviewer 对 final rehydrate diff 的结论：**0 P0-P2**；唯一 P3 是测试名仍写 “verified”，已在生产提交前改名。Outgoing range 初审随后发现两项 P2：pre-travel artifact 时间归因与 raw-run provenance 不完整；前者由 `77eb9380` 修复，后者由本节的 raw commit / manifest snapshot / equivalence proof 修复，等待最终 re-review。
