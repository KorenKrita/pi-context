...
for (const d of readdirSync(RUNS_DIR)) {
  ...
  runs.push({
    model: r.model.modelId, eff: r.thinkingLevel, variant: r.variant,
...
console.log("\n================ 1. 模型综合排行（跨四轮均值）================");
console.log(pad("model", 26) + pad("eff", 6) + pad("轮", 4) + pad("任务", 6) + pad("总分", 6) + pad("时长m", 8) + pad("token", 9) + pad("金额$", 9) + "tier");
const modelRows = models.map((m) => {
  const rs = runs.filter((r) => r.model === m);
  return {
    m, eff: rs[0].eff, n: rs.length,
...
for (const r of modelRows) {
  console.log(pad(r.m, 26) + pad(r.eff, 6) + pad(r.n, 4) + pad(fmt(r.task, 1), 6) + pad(fmt(r.overall, 1), 6) +
    pad(fmt(r.dur, 1), 8) + pad(Math.round(r.tok / 1000) + "K", 9) + pad("$" + fmt(r.cost, 2), 9) + r.tiers);
}

// ---- 2. Version comparison ----
console.log("\n================ 2. 版本对比（跨模型均值）================");
console.log(pad("version", 10) + pad("n", 4) + DIMS.map((d) => pad(d.slice(0, 5), 6)).join("") + pad("overall", 8) + pad("时长m", 8) + pad("token", 9) + pad("金额$", 8));
for (const key of VARIANTS) {
  const rs = runs.filter((r) => r.variant === key);
  if (!rs.length) continue;
  console.log(pad(VLABEL[key], 10) + pad(rs.length, 4) +
...
console.log("\n================ 3. 最适合 ACM 的模型（5 个 ACM 维度均值，排除任务）================");
console.log(pad("model", 26) + pad("轮", 4) + pad("ACM均", 7) + ACM_DIMS.map((d) => pad(d.slice(0, 5), 6)).join("") + "bestVariant");
const acmRows = models.map((m) => {
  const rs = runs.filter((r) => r.model === m);
  ...
  return { m, n: rs.length, acmAvg, byDim: Object.fromEntries(ACM_DIMS.map((d) => [d, avg(rs, (r) => r.dims[d])])), best: best ? `${VLABEL[best.variant]}(${best.overall})` : "-" };
});
acmRows.sort((a, b) => (b.acmAvg ?? -1) - (a.acmAvg ?? -1));
for (const r of acmRows) {
  console.log(pad(r.m, 26) + pad(r.n, 4) + pad(fmt(r.acmAvg, 2), 7) + ACM_DIMS.map((d) => pad(fmt(r.byDim[d], 1), 6)).join("") + r.best);
}
...
for (const r of perfect) console.log(`★ ${r.model} (${r.eff}) @ ${VLABEL[r.variant]}: 六维全 3，六阶段全抓，overall ${r.overall}`);
console.log("\n-- 近满 run（overall==3）--");
for (const r of runs.filter((x) => x.overall === 3).sort((a, b) => b.phasesTaken - a.phasesTaken)) {
  console.log(`  ${r.model} (${r.eff}) @ ${VLABEL[r.variant]}: overall 3, 阶段 ${r.phasesTaken}/6, dims ${DIMS.map((d) => r.dims[d]).join("")}`);
}
