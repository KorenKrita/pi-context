  ...
  return context([
    ...
    [completed("acm_travel", {
      ...
      handoff: handoff({
        ...
        next: `Write recovered-plan.md containing exact remediation nonce ${PRECISE_NONCE}.`,
      }),
    }, {
      target: PRECISE_BASE,
      backupCurrentHeadAs: PRECISE_ARCHIVE,
  ...
  return context([
    ...
    [completed("acm_travel", {
      ...
      handoff: handoff({
        ...
        next: `Create ${REHYDRATE_RETURN} before recovering the archived forensic detail.`,
      }),
    }, {
      target: REHYDRATE_BASE,
      backupCurrentHeadAs: REHYDRATE_ARCHIVE,
...
describe("topology scenario prompt contracts", () => {
  test("does not feed the final travel target as a direct instruction", () => {
    ...
    expect(rehydrate.turns[6]?.prompt).not.toContain(`target ${REHYDRATE_RETURN}`);
    expect(rehydrate.turns[4]?.prompt).toContain("archive recovery pointer");
    expect(rehydrate.turns[6]?.prompt).toContain("semantic return role");
  });
...
describe("rehydrate-round-trip scoring", () => {
  test("requires both off-path directions and the extracted nonce in the returned NEXT", () => {
    ...
    expect(result.checks.find((item) => item.name === "T7 returns to the saved off-path return point")?.pass).toBe(true);
    expect(result.checks.find((item) => item.name === "T7 returned handoff carries recovered exact nonce")?.pass).toBe(true);
  });
  ...
  test("fails if the return handoff omits the recovered nonce even when the branch round trip succeeds", () => {
    const ctx = rehydrateHappyContext();
    const returned = ctx.turnRecords[6]?.toolCalls[0];
    ...
    const result = scenario("rehydrate-round-trip").score(ctx);
    expect(result.pass).toBe(false);
    expect(result.checks.find((item) => item.name === "T7 returned handoff carries recovered exact nonce")?.pass).toBe(false);
  });
});
