...
The habit fires at the seams of the work: a burst of reads distilled into findings; a direction rejected; a phase completed as the next begins; a new request over finished work; summaries starting to stack — and, only as a backstop, when pressure has risen. At each, folding is the default, not an optional extra: when the trail behind you is already sediment, fold it and carry pointers. Skip only when you can name why the raw detail must stay live.
