...
- **NEXT stalls** — required archived detail is missing; fold criteria fail; keep process live or recover the pointer first.
...
Before `acm_travel`, answer in one line: what leaves, what pointer recovers it, and what single action is NEXT. Mixed tool batches are rejected before mutation.
...
Final answer sources the handoff working set; keep archived process detail off the spine.
