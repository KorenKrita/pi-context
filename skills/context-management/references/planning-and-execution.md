...
5. Once the subtask produces a stable takeaway and another subtask or phase remains, travel to the anchor that gives the next subtask the cleanest sufficient working set, often the plan-ready or phase-start anchor.
...
8. If the last subtask completes the user's whole request, give the final answer without an automatic travel; decide on cleanup at the next user message if the conversation continues.
...
Do not travel just because a todo list exists. Travel when a specific execution segment has already served its purpose and can be traveled for an actual continuation. If the segment changed files, launched/stopped processes, or updated external systems, record those side effects in the summary; context navigation does not revert them.
