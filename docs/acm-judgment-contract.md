...
折叠是你安排的一步，不是你感知的一刻（感知层只有仪表，宪法第 2 条）。当你划分接下来的工作时，折叠点和测试、提交一样是步骤的候选：在提取将会闭合的位置安排它。当一个新请求到来，而它只用得上上一段的结论、用不上它的过程时，上一段就已经在候选位置上。判断「这里没有值得折的点」同样是完整的判断。折叠属于你的工作安排，不属于给用户的叙述。折叠点之间持续压缩（把观察随手整合成结论），让折叠步骤到来时提取已经闭合；成批折叠，以有意义的单元提交表示收益。ACM 操作本身是 overhead 不是 deliverable：不叙述、不仪式化，工作本身始终是主题。
