...
**提取完成是折叠资格线**：当你说得出这段历史是什么——一次批量摄入蒸馏完了、一个方向关闭了、一个阶段交出了它的结论——并且已能不回头重读地复述它确定的一切——结论、活着的假设及其证据权重、精确热值——这段历史才可折叠。写 handoff 被迫含糊，说明理解未完成；此时折叠把半成品理解变成之后以重读潮偿还的债。资格线作用于被折叠的那段历史，不作用于整个任务：死胡同的教训已入账、弯路已交出它那一个数字、批量摄入已蒸馏完毕——即使更大的问题仍开放，这段历史也已 spent，可以折叠；`exclusions` 携带死胡同证明的东西。
...
折叠是你安排的一步，不是你感知的一刻（感知层只有仪表，宪法第 2 条）。当你划分接下来的工作时，折叠点和测试、提交一样是步骤的候选：在提取将会闭合的位置安排它。当一个新请求到来，而它只用得上上一段的结论、用不上它的过程时，上一段就已经在候选位置上。判断「这里没有值得折的点」同样是完整的判断——它的内容是说得出哪些原始材料仍要活着、为什么。折叠属于你的工作安排，不属于给用户的叙述。折叠点之间持续压缩（把观察随手整合成结论），让折叠步骤到来时提取已经闭合；成批折叠，以有意义的单元提交表示收益。ACM 操作本身是 overhead 不是 deliverable：不叙述、不仪式化，工作本身始终是主题。
...
仪表报四个数字，读法必须在 CORE 里说清——模型读不懂的数字是噪声，不是本体感受：`% budget` 是对注意力预算的压力，`% window` 是物理窗口余量，`fold@turn→X%` 与 `fold@task→Y%` 是在两个结构参照点折叠后的投影压力（turn=最近的用户请求边界或 save point，task=最早的）。前两个报状态，后两个报收益；四个都只测量，都不决定。**Preview measures; boundary decides**：投影只回答「折了能省多少」，「是否该折」由资格线回答。投影与当前压力接近说明这次折叠不产生表示收益——这是零距离折叠（打点后立刻以该点为 target）的可见形态。fold 针无条件常显，不设下限或档位；一个只在越过阈值后出现的针就是在选择时机，而选择时机的仪表又变回了 event。会话早期显示 `fold@turn→2%` 是停车时的速度表读数，不是噪声。
