...
- `src/lib.ts:382-449`
- `src/message-sanitizer.ts:37-69`
...
- 比较当前 meaningful backup 与最深 protocol-complete prefix；
- 恢复两种 alias，断言 LLM-bound packet 是否需要 protocol repair；
- 再用行为实验比较旧工具重放率。

**正确性质**:
Travel backup 保存的是可直接恢复的 raw continuation leaf，应落在当前 travel call 之前最深的 protocol-complete prefix。普通 checkpoint 的语义 target contract保持独立。
