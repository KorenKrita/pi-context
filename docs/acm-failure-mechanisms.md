...
**状态**: Confirmed interface friction; mechanically resolved; controlled strong/weak outcome matrix passed; production rate unknown
...
**解决方式**:
`acm_travel` 接收七字段 structured object；`src/handoff.ts` 验证 authoritative/supporting fields、组合 verified raw archive fact，并生成唯一 canonical text。真实弱模型 run 观察到 provider 会把 nested object 整体编码为 JSON string，因此 runtime 也接受同一七字段对象的精确 JSON 编码，再走相同验证；Legacy free-form `summary` 与自由文本 DSL 不再作为生产接口。
...
**状态**: Historical unavailability confirmed; eval availability plane resolved and provenance-gated
...
**底层机制**:
旧 bare mode 使用 `--no-skills`。旧 full-env mode 删除已安装的 `pi-context` package，只用 `-e src/index.ts` 注入 extension；该加载方式不会带入 package manifest 声明的 `skills/`。当前 harness 将环境显式分为 core-only、product-isolated 与 full-env；后两者都从当前 checkout 注入两个 extension 与 exact Skill 文件。
...
**当前解决方式**:
每个 run 在首个模型 prompt 前调用 `get_commands`。Core-only 必须得到 0 个 `skill:context-management`；product-isolated/full-env 必须恰好得到 1 个，并以 realpath 证明来自当前 checkout。失败 run 标记 `infrastructure_invalid`，不发送任务 prompt、不执行 judge、不归因模型。
...
- `eval/evidence/advanced-pointer-routing-matrix-2026-07-20.json`

## FM-04 — Progressive-disclosure pointer miss

**状态**: Historical pointer mismatch confirmed and mechanically resolved; controlled pointer-routing matrix passed; spontaneous production frequency unknown

**现象**:
用户报告真实使用中 agent 很少主动加载 advanced Skill。当前接口可以确认 pointer 名称、触发层级与双跳读取存在摩擦，但尚未隔离这些因素各自对 production non-use 的因果贡献。

**底层机制**:
历史 runtime cue 使用 `Advanced Target Selection` 等内部标题，与 available Skill 名称 `context-management` 不一致。当前 timeline/rebase 与 exceptional recovery cue 使用 exact Skill name 和 exact reference path；router → reference 的渐进披露仍保留。
...
**当前解决方式与证据边界**:
Runtime 只在可观察的 advanced 条件中给 exact pointer，不推断语义复杂度；它先通过 Pi 的公开 <code>getCommands()</code> 确认 `skill:context-management` 实际可用，core-only 根本不会收到该 pointer。`advanced-pointer-routing` analogue 的 corrected scorer 只接受有 matching completion 的成功 read，并把 Skill docs/find/grep/bash probe 计为 core-only isolation failure。`gpt-5.6-sol/high` 与 `deepseek-v4-flash/high` 在 product-isolated 均读取 router 与 target-selection reference；core-only 均无 advanced probe 且保守不 travel。该 4/4 证明 availability + condition-specific pointer 链路，不证明无显式 analogue 的 production 自发触达率。
...
**当前解决方式**:
Travel 在 mutation 前生成独立 target facts：target entry/role/stop reason、assistant tool/text shape、surviving latest user 是否 open、protocol complete/repaired/invalid 与 repairs/defects、branch-summary 和 off-path。Staged policy 将 `invalid` tool-call identity 作为唯一已确定 hard floor，直接以 `target_protocol_invalid` 拒绝且不创建 backup/summary；repaired、dangling、assistant tool batch、old summary 与 off-path 进入 structured warnings，不自动改 target，也不伪装成 semantic verdict。由于 repaired 包含 missing-result、reorder、orphan/duplicate cleanup 等异质形状，未来升级应按 subtype 与 controlled outcome，而不是 blanket reject。
...
**状态**: Historically confirmed behavior; structurally observable open-turn mitigation implemented and deterministically covered; semantic enforcement remains intentionally out of scope

**现象**:
历史样本中，handoff `State` 已包含当前答案，但 `Goal` 或 `NEXT` 写成“无待办”“等待下一请求”，travel 后模型没有交付。2026-07-20 的 shrunk research flow 仍复现：P3 handoff 带完整官方结论，却写成“no answer outstanding”，travel 后只说等待下一问题，judge 将 task completion 降级。

**底层机制**:
Handoff 结构不能可靠地语义判断“答案是否已经向用户交付”，但 session topology 可以判断 travel tool batch 前的 latest user turn 是否仍没有 visible assistant response。模型仍可能把“已经知道答案”误判成“已经交付”。
...
- `eval/PHASE9-LOG.md`
- `eval/PHASE10-LOG.md`
- `eval/evidence/cadence-flow-outcome-progression-2026-07-20.json`
- `eval/evidence/cadence-raw-control-pair-2026-07-20.json`
- `eval/evidence/cadence-raw-control-pair-2026-07-20.json`

## FM-08 — Pressure reminder carries a travel-permission gradient

**状态**: Historical action-oriented wording and harm confirmed; judgment-only replacement has healthy and unhealthy production-like fossils, not a single-variable causal estimate
