...
**状态**: Confirmed interface friction; mechanically resolved by structured input; model-outcome evidence pending
...
**解决方式**:
`acm_travel` 接收七字段 structured object；`src/handoff.ts` 验证 authoritative/supporting fields、组合 verified raw archive fact，并生成唯一 canonical text。Legacy free-form `summary` 不再作为并行生产接口。
...
**状态**: Confirmed host framing; mechanically mitigated by marked in-place Context Packet projection; model-outcome evidence pending
