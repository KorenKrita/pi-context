...
export function registerTravelTool(pi: ExtensionAPI, runtime: AcmSessionRuntime): void {
  const schema = Type.Object({
    target: Type.String({ minLength: 1, maxLength: 256, description: "Checkpoint name, history node ID, or 'root'. Choose the last clean anchor before the compression seam, not the nearest or most conveniently named anchor. For a rebase, compare candidates from earliest to latest and choose the first that retires competing active handoffs without growing projected summary depth and whose handoff preserves continuation fidelity. Root is a candidate, not a default. Use acm_timeline checkpoints/search for comparison and tree only when ancestry or representation ownership remains ambiguous." }),
    summary: Type.String({ minLength: 1, maxLength: 10000, description: `Authoritative representation that becomes the working set after travel. A fresh agent must be able to continue the current cognition from this handoff and direct evidence pointers without archived conversation. Fill every slot once and in order; write 'none' rather than omitting a category: ${HANDOFF_SLOT_HINT}. State may encode knowns, unknowns, hypotheses, attribution, constraints, hot details, discriminators, identifiers, and surviving fronts. Preserve external effects, exclusions, provenance, and recovery pointers; pointers over process dumps. Max 10000 chars.` }),
    backupCurrentHeadAs: Type.Optional(Type.String({ minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9._-]+$", description: "Optional semantic archive label for the raw path being replaced. The structural target keyword 'root' is reserved in every letter case. Use a unique name that makes omitted precision discoverable. This label creates recoverability; its spelling does not classify the fold, prove completion, select the target, or replace an authoritative seven-slot handoff." })),
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      if (!findInTree(tree, (node) => node.entry.id === targetId)) {
        const hint = " Use acm_timeline to choose the last clean node before the process you want to replace; raw node IDs are valid targets.";
        return {
