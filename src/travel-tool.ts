...
export function registerTravelTool(pi: ExtensionAPI, runtime: AcmSessionRuntime): void {
  const schema = Type.Object({
    target: Type.String({ minLength: 1, description: "Checkpoint name, history node ID, or 'root'. The point to travel to — pick the node before the material you're folding. Use acm_timeline to find node IDs." }),
    handoff: HandoffSchema,
    backupCurrentHeadAs: Type.Optional(Type.String({ minLength: 1, pattern: "^[A-Za-z0-9._-]+$", description: "Optional name to save the current position before traveling. Creates a recovery point you can return to later. ('root' is reserved)." })),
