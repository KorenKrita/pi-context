...
export function registerTravelTool(pi: ExtensionAPI, runtime: AcmSessionRuntime): void {
  const schema = Type.Object({
    target: Type.String({ minLength: 1, description: "Checkpoint name, history node ID, or 'root'. Choose the last clean node before the material being folded, not the nearest or best-named label. An alias created by backupCurrentHeadAs is a raw archive origin: it normally restores or expands pre-travel history, so target it only for deliberate rehydrate/restore and never as a fold/rebase base. For a rebase, compare candidates from earliest to latest and take the first whose projected summary depth does not grow and whose handoff passes cold start; root is a candidate, not a default. On large trees use acm_timeline with view checkpoints or search; use view tree only when ancestry matters." }),
    handoff: HandoffSchema,
    backupCurrentHeadAs: Type.Optional(Type.String({ minLength: 1, pattern: "^[A-Za-z0-9._-]+$", description: "Optional brand-new recovery alias for the latest protocol-complete point on the current origin path before mutation. This field never selects the destination. Put an existing checkpoint, archive alias, or return alias in target and omit backupCurrentHeadAs. Use this field only when a new alias is needed for this exact pre-travel path. The name must be unique and semantic, e.g. latency-hunt-scan or parser-attempt-2; the structural target keyword 'root' is reserved in every letter case. The name is never a workflow state or a substitute for a cold-start handoff." })),
