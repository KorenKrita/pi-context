...
function trustedContinuationQueues(
  ...
  // tool_result handler. A summary whose receipt (matched by the travel's tool call id, wherever it
  // landed on the branch) exists but is not trusted stays archival.
  const trustedSummaries = new Set(trustedTransactions.map((transaction) => transaction.summaryEntryId));
  const receiptCallIds = new Set(entries.flatMap((entry) => (
    entry.type === "message" && entry.message.role === "toolResult" && entry.message.toolName === "acm_travel"
      ? [entry.message.toolCallId]
      : []
  )));
  for (const entry of entries) {
    const receiptCallId = entry.type === "branch_summary" ? record(entry.details)?.toolCallId : undefined;
    if (typeof receiptCallId === "string" && receiptCallIds.has(receiptCallId) && !trustedSummaries.has(entry.id)) continue;
    if (
      entry.type !== "branch_summary"
      || typeof entry.summary !== "string"
