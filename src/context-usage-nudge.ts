...
export function buildContextUsageNudgeMessage(nudge: PendingContextUsageNudge): ContextUsageNudgeMessage {
  ...
  const guidance = nudge.level === 30
    ? [
        "Continue the current work normally. At the next natural semantic boundary, check decision smells and whether the fold gate can pass against the working-set invariant.",
    ...
    : nudge.level === 50
      ? [
          "Context pressure is becoming material. Raise the priority of a rebase check, and look for the next recognizable moment where the fold gate and cold start can pass.",
      ...
      : [
          "Context pressure is high. At the earliest safe semantic boundary, prefer a fold or rebase when the fold gate and cold start pass.",
