...
export const StructuredHandoffSchema = Type.Object({
  goal: Type.String({
    minLength: 1,
    description: "The current objective — what you're trying to accomplish.",
  }),
  state: Type.String({
    minLength: 1,
    description: "What you know, what's uncertain, key files/values/identifiers. Be specific.",
  }),
  evidence: Type.String({
    minLength: 1,
    description: "File paths, commands, IDs that support your state. Write 'none' if empty.",
  }),
  external: Type.String({
    minLength: 1,
    description: "Files changed, processes started, external system state. Write 'none' if empty.",
  }),
  exclusions: Type.String({
    minLength: 1,
    description: "Dead ends and closed directions — don't revisit these. Write 'none' if empty.",
  }),
  recover: Type.String({
    minLength: 1,
    description: "Checkpoint names or node IDs for recovery. Write 'none' if empty.",
  }),
  next: Type.String({
    minLength: 1,
    description: "The exact next action — concrete and immediately executable.",
  }),
...
export const HandoffSchema = Type.Union([
  ...
  Type.String({
    minLength: 1,
    description: "JSON fallback: exact JSON encoding of the same 7-field object. Not free-form text.",
  }),
], {
  description: "Use the structured 7-field object. A JSON string is accepted as fallback.",
});
