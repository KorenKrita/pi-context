...
export const GUIDANCE_CUES = {
  ...
  "timelineCheckpoints": "把折叠目标选在要折的那段内容之前——取最近的一个干净点。标 raw archive 的存档指向折叠前的完整历史，用于找回旧细节，别当折叠目标。",
  "timelineSearch": "搜索覆盖整棵树，含已折叠的历史。结果里的节点 ID 可直接作为 acm_travel 或 acm_checkpoint 的目标。",
  "timelineTree": "分支结构用来判断哪个点在前、哪个点在后；别把折叠目标选在要折的范围里。"
} as const;
