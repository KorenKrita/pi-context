...
describe("canonical guidance generation", () => {
  test("derives every artifact from the two marked sources", () => {
    ...
    expect(RECOVERY_GUIDANCE.hostCapability).toContain("unavailable");
  });
  ...
  test("keeps manual navigation summaries handoff-shaped and standalone", () => {
    ...
    expect(TREE_SUMMARY_INSTRUCTIONS).toContain("conversation branch");
    expect(TREE_SUMMARY_INSTRUCTIONS).toContain("file paths");
  ...
  test("keeps result cues concise and view-specific", () => {
    ...
    expect(GUIDANCE_CUES.rebaseCheck).toContain("summary");
    expect(GUIDANCE_CUES.rebaseCheck).toContain("rebas");
    expect(GUIDANCE_CUES.timelineActive).toContain("spine");
    expect(GUIDANCE_CUES.timelineCheckpoints).toContain("save points");
    expect(GUIDANCE_CUES.timelineSearch).toContain("tree");
    expect(GUIDANCE_CUES.timelineTree).toContain("topology");
  });
