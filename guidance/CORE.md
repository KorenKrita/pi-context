...
- **acm_timeline** — view the working set, save points, and usage; search across the tree, folded history included; the tool says when its scan was bounded.
...
A stretch is ready when it is digested; phase transitions are where that most often becomes visible — the moment the next piece of work needs different things in view than the last one did: exploration ends and editing begins, a plan settles into execution, a diagnosis becomes a fix, one component closes and the next opens, a result becomes ready to deliver. Each transition is a moment to ask the one question. The question alone decides. Readiness also arrives mid-phase — a search that keeps returning the same ground has already confirmed what stands and what is ruled out. A run of similar items — reviews, tickets, cases — has its own cadence: the first item settles the method, and each finished item after it is a stretch whose verdict fits one line; fold there, carrying the method and the verdicts, and open the next item light.

Exploration converged and the conclusions with their file:line anchors can all be stated without rereading → fold, and the first edit is clear. The same question can return vague: files read, each interesting, but the handoff cannot yet be written without going back — continuing is the test working, not a fold missed.
...
**Opening a long stretch.** About to pile up raw material — a batch of files, a long investigation, a path that may not survive — set a checkpoint first, named for what you are about to do. That mark is where the stretch begins; when the conclusions are distilled — the same fold test — fold to it. When an attempt dies and what it ruled out is concrete without rereading, travel back to the mark carrying that in `exclusions` and any lasting side effects in `external`; what the attempt proved along the way — verified facts, working artifacts, ground confirmed solid — rides in `state` for the next approach to build on. Either way, only the conversation detail leaves the working set.

**The handoff.** `goal`, `state`, `next` are required; `evidence`, `external`, `exclusions`, `recover` only when they carry something. Write it for a fresh continuation: the surviving hypotheses, exact values, and the next action, stated so they stand alone. Anything cheaply refetchable rides as its retrieval pointer — the file path and line, the command, the query, the id; a value goes in verbatim when it is small, volatile, or needed for the very next step. When the way forward waits on the user's decision, `next` is the question to ask. A routine fold needs three fields:
...
One fold, start to finish: a refactor request arrives and the module is unfamiliar, so a checkpoint named `refactor-scan` marks the start before the files are opened. Files read and searches run, the structure is clear. The goal with its constraints, the file:line conclusions for `state`, and the first edit for `next` all come without rereading — the fold test passes. Travel to `refactor-scan` carries them; the edits proceed in the light working set. Later a config default recorded only in folded history is needed; timeline search returns it, and the work continues.
...
The gauge reports where the session stands; the fold test alone decides readiness.
